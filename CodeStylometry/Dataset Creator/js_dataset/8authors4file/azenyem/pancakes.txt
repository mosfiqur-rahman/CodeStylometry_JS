...
//    console.log("*" + pancakes[i]);
  }
/*
  console.log("Calc " + caseNumber);
  for (key in nums.nums) {
    console.log("[" + key + "] : " + nums.nums[key]);
  }
/**/
...
function calculate2(nums) {
  ...
  for (var targetHeight = nums.maxNum; targetHeight >= 2 ; targetHeight--) {//mennyi maradhat
...
//      console.log("i: " + i + " c: " + c + "added: " + ((Math.ceil(i / targetHeight) - 1) * n.nums[i]));
      if (c > minSoFar) {
        return c;
      }
    }
  }
//  console.log("c returned: " + c);
  return c;
}
